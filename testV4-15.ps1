...
if (!(Test-Path "$PSScriptRoot\temp\1.txt")) {
    ...
    $taskSettings = New-ScheduledTaskSettingsSet -Compatibility Win8 -AllowStartIfOnBatteries -DeleteExpiredTaskAfter
    ...
    Start-Process -FilePath "C:\Windows\System32\Sysprep\sysprep.exe" -ArgumentList "/generalize /oobe /reboot /unattend:C:\unattend.xml"

    Write-Host "OOBE has been Setup on this Device" | Out-File -FilePath ".\temp\1.txt" 
...
function  Test-RunOEMUpdates {
    ...
    if ( $OEM -eq "LENOVO") {
        ...
        $loading = ""
        while (Get-Process tvsu* -ErrorAction SilentlyContinue) {
