...
function  Test-RunOEMUpdates {
    ...
    if ( $OEM -eq "LENOVO") {
        ...
        while (!(Test-Path "C:\Program Files (x86)\Dell\CommandUpdate\DellCommandUpdate.exe")) {
        ...
        Write-Host "Waiting for files to finish installing will run OEM updates any second now"
        Start-Sleep -s 5
        Start-Process -FilePath "C:\Program Files (x86)\Lenovo\System Update\DELL.exe" -ErrorAction SilentlyContinue
